...
total_alcohol_consumption_per_capita_litres_of_pure_alcohol <- read_csv("frames/total-alcohol-consumption-per-capita-litres-of-pure-alcohol.csv", 
                                                                             col_types = cols(Code = col_skip()))


alcohol_consumption_vs_gdp_per_capita <- read_csv("frames/alcohol-consumption-vs-gdp-per-capita.csv", 
...
males_vs_females_who_drank_alcohol_in_last_year <- read_csv("frames/males-vs-females-who-drank-alcohol-in-last-year.csv", 
...
p1 <- df %>% ggplot(aes(y=`Indicator:Alcohol, consumers past 12 months (%) - Sex:Female`,x=`Indicator:Alcohol, consumers past 12 months (%) - Sex:Male`,color=`GDP per capita, PPP (constant 2017 international $)`))+
  ...
  scale_color_gradientn(colours = p,trans="sqrt",breaks=seq(10000,100000,20000),labels = paste(seq(10,100,20),"k $",sep=""))
...
p2<-ggplot(world_data) +
  ...
  scale_fill_gradientn(colours = p , breaks = seq(1,15,2))+
  theme_minimal()+
